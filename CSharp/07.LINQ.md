...
- where 절은 데이터 소스의 어떤 요소가 쿼리 식에서 반환될지를 지정하기 위해 사용
- 각 소스 요소에 bool 조건을 적용하고 지정된 조건이 참인 요소를 반환 
- 단일 쿼리 식에서는 여러 where 절을 포함할 수 있고 여러 조건부 하위 식을 포함할수 있음

```cs
// ex) where절로 5보다 작은 숫자를 제외한 모든 숫자 필터링 
int[] num = { 5, 4, 1, 3, 9, 6, 7, 8 };
